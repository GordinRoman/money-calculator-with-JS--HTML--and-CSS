...
const totalBalance = document.querySelector('.total__balance'),
    ...
    plusButton = document.querySelector('.operation__add_plus'),
    minusButton = document.querySelector('.operation__add_minus'),
    operationName = document.querySelector('.operation__name'),
    operationAmount = document.querySelector('.operation__amount');

let dataBaseOperation = JSON.parse(localStorage.getItem('calc')) || [];

//Добавление операции дохода/расхода в HTML
...
const toMinus = () => {
    operationAmount.value = -operationAmount.value;
};

//Создание операции расхода/дохода
const addOperation = (event) => {
    ...
    if (operationNameValue && operationAmountValue) {
        ...
        if(!(operationAmountValue && -operationAmountValue)) operationAmount.style.borderColor = 'red';
    }   
    operationName.value = '';
...
minusButton.addEventListener('click', toMinus);
minusButton.addEventListener('click', addOperation);
